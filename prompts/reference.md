...
- **Show ties in numeric scores; drop them when the score is spelled out.** A matchup result written with digits always carries all three numbers, ties included: `6-5-1`, `9-3-0`, `5-6-1` (W-L-T, summing to 12) — this applies everywhere (recaps, power rankings, hot takes). But when a score is rendered in words, give only the two sides and omit the tie: write "nine to nothing" or "nine-nothing" (never "nine-zero-three"), "ten to nothing" (never "ten-nothing-two"). Never spell the tie count out in words
