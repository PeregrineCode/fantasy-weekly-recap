...
function parsePlayerInfo(playerArray) {
  ...
  // player_stats, selected_position, and percent_owned live at varying indices
  // depending on the endpoint (roster puts selected_position at [1], stats at [2];
  // league players puts stats at [1] with no selected_position). Scan all indices.
  let stats = {};
  ...
  for (let i = 1; i < playerArray.length; i++) {
    ...
    if (Array.isArray(playerArray[i]?.percent_owned) && ownership === 0) {
      const valObj = playerArray[i].percent_owned.find(x => x?.value !== undefined);
...
async function fetchTeamRoster(client, teamKey) {
  const data = await client.get(`/team/${teamKey}/roster/players;out=stats,percent_owned`);
